...
depends_on: [ENV-14, ENV-30]
blocks: [ENV-90, ENV-91, ENV-92, ENV-100]
package: core
prd: [§6.10]
estimate: M
---

# ENV-80 — Public JSON in/out API (load / getDoc / change)
...
that ENV-30 stubbed — `loadDoc(doc)`, `getDoc()`, and a `change` event emitting the
current doc — using the doc-model codec (ENV-14) for validation on load and the doc as
the single source of truth. The React/Vue wrappers (ENV-90/91) and demo (ENV-92) all
...
to every edit — all validated through ENV-14.

## Prerequisites
- ENV-14 done (`serialize`/`deserialize`, `validateDoc` via the codec — use these;
  don't re-validate by hand).
- ENV-30 done (the `loadDoc`/`getDoc`/`change` stubs + `EnveloppeConfig`).
- The editor's internal doc state + the edit pipeline (ENV-11/32/62 emit new docs).
...
1. **Internal source of truth.** The editor holds the current `EnveloppeDoc` (one
   immutable doc; edits replace it with a new doc + patch from ENV-11). Every consumer
   (canvas ENV-32, panel ENV-62) reads/writes through this single field.
...
5. **Initial doc.** If no doc is loaded, start from `createEmptyDoc()` (ENV-10 factory)
   so the editor is always in a valid state.
6. **Wrapper-friendliness.** Keep the surface plain DOM (methods + a `CustomEvent`) so
   the React/Vue wrappers (ENV-90/91) map props/events/refs onto it without special
   cases. `getDoc()` + `change` enable controlled-value usage.
7. **Budget** — no new runtime dep; reuse `@enveloppe/doc-model`.

## Acceptance criteria
- [ ] `loadDoc(doc)` validates via ENV-14, replaces editor state, repaints the canvas,
...
## Out of scope
- React/Vue wrappers (ENV-90/91) — they consume this surface.
- `onImageUpload` (ENV-81). Any backend/storage (headless — host owns it).
- The doc codec internals (ENV-14).
