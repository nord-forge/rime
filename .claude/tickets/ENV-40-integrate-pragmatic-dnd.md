...
depends_on: [ENV-32, ENV-33]
blocks: [ENV-41, ENV-43, ENV-44, ENV-46, ENV-47]
package: core
prd: [§6.6]
estimate: L
---

# ENV-40 — Integrate Pragmatic drag-and-drop
...
**inside the iframe** — so every pointer position must go through the ENV-33
`DragCoordinateController`, and every successful drop must mutate the doc through
ENV-11 immutable operations (never by poking the DOM directly). This ticket
establishes the wiring + drop semantics; the *performant* hit-testing + visual
indicators are ENV-41, and a11y/keyboard is ENV-44/45.
...
ENV-11 patch that updates the doc (and re-renders via ENV-32), in Chromium +
WebKit.

## Prerequisites
- ENV-32 done (`CanvasRenderer` paints the doc; every element has
  `data-node-id` + `data-node-type`; `elementForNode`/`nodeIdAt` exist).
- ENV-33 done (`DragCoordinateController` — `hostToCanvas`, `nodeIdAtHostPoint`,
  `isOverCanvas`, `invalidate`).
- ENV-11 done (`insertNode`, `moveNode`, `removeNode` returning
  `{ doc, patch, inverse }`).
...
2. **`dnd-controller.ts`** — `DndController`, the single owner of the DnD wiring.
   ...
   `dispatch` callback that applies an ENV-11 operation result to the editor
   state (so the editor owns the doc + undo stack, ENV-12).
   ```ts
   ...
   export interface DndDeps {
     ...
     dispatch: (op: OpResult) => void; // OpResult = { doc, patch, inverse } from ENV-11
   }
   ...
   export class DndController {
     ...
     destroy(): void; // tear down ALL registered cleanups (memory; ENV-47)
   }
   ```
3. **Drop target resolution** — on every drag move over the canvas, compute the
   ...
   - Nested zones: if the pointer is over a section gap (between columns) resolve
     ...
     unit-testable with injected rects (the *fast/throttled* version is ENV-41 —
     here, correctness first).
4. **Drop → doc mutation (ENV-11 only)** — on `onDrop`:
   ```ts
   ...
   Reject drops that would make an invalid doc (ENV-11 throws → swallow + no-op).
...
- [ ] Every drop goes through an ENV-11 op (`{ doc, patch, inverse }`); no direct
...
## Out of scope
- Throttled/rAF hit-testing + insertion indicators (ENV-41).
- Perf budget measurement (ENV-42), custom drag preview (ENV-43).
- Keyboard reordering + ARIA announcements (ENV-44/45).
- Touch-specific E2E (ENV-46) — basic pointer path here.

## Verification
```bash
cd packages/core
bun test     # resolveDropTarget midpoint/empty-column cases; drop → correct ENV-11 op called
bun run build
bun run lint
...
all drops via ENV-11; bundle still ≤ budget (note Pragmatic's gzip cost);
status → `review`.
