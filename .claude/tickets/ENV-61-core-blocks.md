...
depends_on: [ENV-60]
blocks: [ENV-65, ENV-81, ENV-92]
package: core
prd: [§12]
estimate: L
---

# ENV-61 — Core blocks via registerBlock

## Context
These are the v1 "done-bar" blocks (§12): Section, Column(s), Text, Image, Button,
Divider, Spacer. The whole point of ENV-60 is that built-ins ship through the **same**
...
them, with no internal shortcut. This is also the reference every custom block (ENV-64)
imitates.

## Goal
`@enveloppe/core` registers the seven core blocks through `registerBlock`, each with a
schema (drives ENV-62), palette entry (feeds ENV-63), canvas renderer (paints into the
ENV-31 iframe), and export renderer (emits MJML matching ENV-21), wired by a single
`registerCoreBlocks()` the editor calls on init.

## Prerequisites
- ENV-60 done (`registerBlock`, `BlockDefinition`, `BlockSchema`/`FieldDef`,
  `PaletteEntry`, canvas/export render contexts).
- ENV-10 node types (`SectionNode`/`ColumnNode`/`TextBlock`/`ImageBlock`/`ButtonBlock`/
  `DividerBlock`/`SpacerBlock`, `BlockStyle`, `RichTextJSON`) and factory helpers.
- ENV-32 (`applyStyle`, the divs/flex canvas conventions — reuse, don't re-invent).
- ENV-21 (the MJML mapping table + `styleToMjmlAttrs` — mirror it for core export).
...
2. **The seven blocks** — schema/canvas/export per block:
   ...
   - **Section/columns:** the section's column-count control mutates the doc by
     adding/removing `ColumnNode` children (via ENV-11 ops in ENV-62); the block here
     just renders whatever columns exist and declares the schema field. Columns in a
     section sum to 100% (ENV-10 invariant) — when count changes, distribute evenly.
   - **Canvas = divs/flex, never tables** (§6.2). **Export = MJML** mirroring ENV-21.
3. **Stay DRY with ENV-21.** Core export handlers must produce MJML identical to
   ENV-21's hardcoded mapping for the same node. Import the shared
   `styleToMjmlAttrs`/`richTextToInlineHtml` from `@enveloppe/renderer-mjml` (or a
   shared util) rather than duplicating — document the single source. ENV-21's
...
4. **`registerCoreBlocks()`** in `core/index.ts` registers all seven on the default
   ...
   (ENV-63), not by skipping registration.
5. **Defaults via factory.** `palette.defaults` should align with ENV-10's factory
...
- [ ] Each block's `renderExport` emits MJML matching ENV-21's mapping for the same
...
- [ ] Unit tests cover schema, canvas DOM, export MJML, and ENV-21 parity per block
      (`bun test`). Core bundle within budget.

## Out of scope
- Social block (ENV-65) — separate, P2. The properties-panel UI (ENV-62) and palette
  UI (ENV-63) — these supply schema/palette data only.
- `onImageUpload` wiring for the image block (ENV-81) — image takes a plain `src` here.
- Merge tokens in text (ENV-70).

## Verification
```bash
cd packages/core
bun test     # per block: schema, canvas DOM (no table), export MJML, ENV-21 parity
bun run build
bun run lint
...
matches ENV-21; size gate green; status → `review`.
