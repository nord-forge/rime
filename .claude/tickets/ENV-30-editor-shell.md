...
depends_on: [ENV-01]
blocks: [ENV-31, ENV-34, ENV-80, ENV-90, ENV-91]
package: core
prd: [§6.5]
estimate: M
---

# ENV-30 — `<enveloppe-editor>` Lit component shell
...
- `@enveloppe/doc-model` exists for the `EnveloppeDoc` type (ENV-10); if not yet
...
2. **Config application** — when `config.theme` is set, apply each `--eb-*` pair to
   ...
   only theming channel for chrome (see ENV-34). Do NOT read host stylesheets.
3. **Public surface, stubbed** — declare the methods later tickets fill so the type
   ...
   `getDoc(): EnveloppeDoc` (wired in ENV-80), and a `change` CustomEvent contract
   (`detail: { doc }`). Stub bodies are fine; document them as ENV-80's job.
4. **Empty regions for now** — palette/canvas/properties render empty containers
   (and their named slots). ENV-31 mounts the iframe into `part="canvas"`; ENV-62/63
...
## Out of scope
- The iframe canvas itself (ENV-31) and doc rendering (ENV-32).
- Palette (ENV-63), properties panel (ENV-62), DnD (ENV-40), rich text (ENV-50).
- Real `loadDoc`/`getDoc`/change-event wiring (ENV-80).
...
bun run build
bun run lint
# browser (uses the ENV-04 harness):
