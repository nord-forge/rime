...
depends_on: [ENV-30]
blocks: [ENV-32, ENV-33]
package: core
prd: [§6.4]
estimate: M
---

# ENV-31 — Same-origin srcdoc iframe canvas
...
2. **srcdoc skeleton** — assign `iframe.srcdoc` to a minimal, self-contained
   ...
   *,*::before,*::after{box-sizing:border-box} html,body{margin:0} body{font:15px system-ui;background:#fff}</style>
   ...
   ```
   The renderer (ENV-32) draws into `#eb-root`; `#eb-base` is the swappable email
...
5. **CSS isolation is structural, not host-leaked** — the iframe must **not** copy or
   ...
   boundary by design (ENV-34 verifies). The only styles inside are `#eb-base` plus
   whatever ENV-32 writes into `#eb-root`.
6. **Wire into the shell** — `EnveloppeEditor.firstUpdated` constructs a
   ...
   `whenReady()` promise for ENV-32 to await. Expose the iframe as `part="canvas-frame"`
   for the ENV-04 `canvasFrame()` helper.
...
## Out of scope
- Rendering the doc into `#eb-root` (ENV-32).
- Coordinate translation / pointer math (ENV-33).
- The full no-bleed theming verification matrix (ENV-34 owns the dedicated test).
