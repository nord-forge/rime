...
depends_on: [ENV-19]
blocks: [ENV-21, ENV-25]
package: core
prd: [§6.6]
estimate: L
---

# ENV-20 — Drop-zone detection + insertion indicators
...
6. **Cleanup** — the indicator element and any rAF must be torn down on drag end
   and on `DndController.destroy()` (no orphaned frame loop; ENV-26 verifies).
...
## Out of scope
- The actual ms latency budget + measurement on the reference machine (ENV-21).
- Custom drag preview (ENV-22), keyboard path (ENV-23), touch E2E (ENV-25).
