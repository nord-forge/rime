...
# ENV-13 — CRDT-friendly shape audit (design only)
...
is a **design-only audit** — it ships **no runtime feature**. It documents where the ENV-10
node identity + children-ordering choices will or won't block a future CRDT, and records
concrete recommendations as an ADR so later collaboration work (and any change to ENV-10/11)
...
references to the ENV-10 types, what is CRDT-ready, what is a risk, and what (if anything)
should change now versus later.

## Prerequisites
- ENV-10 done (the `types.ts` node tree, `NodeId`, `children[]` arrays exist to audit).
...
1. **Status / context** — "design only, v1 ships no CRDT; this records readiness." Link back to
   PRD §6.1 and ENV-10.
2. **What's already CRDT-friendly** — cite the actual ENV-10 shapes:
   - **Stable node identity (✓).** Every node has a required, unique `NodeId` (ENV-10
...
3. **The ordering concern (the real risk).** `children[]` is a plain JS array (ENV-10
   `DocumentNode.children`, `SectionNode.children`, `ColumnNode.children`). Plain arrays are
   **index-addressed**, and ENV-11 patches use numeric `index` for `insert`/`remove`/`move`.
   ...
   - ENV-11's `move` op (`from`/`to` index) is the operation most at odds with a CRDT; note that
   ...
   - Recommend that nothing in v1 should rely on array **index** as a stable identity (always
     resolve by `id`); confirm ENV-11 operations are id-addressed at the API surface (they are:
...
4. **Where a CRDT would attach** — a small diagram/table: `document` map → `children` `Y.Array` →
   ...
   ENV-10 `RichTextJSON`) → maps to `Y.Text`/`Y.XmlFragment`; note the portable RichTextJSON
   shape is convertible but is **not itself** a CRDT (the editor engine, ENV-50, owns live
...
6. **Decision** — one line: the ENV-10 shape is CRDT-ready **provided** the array-ordering and
   id-uniqueness notes above are honored; no ENV-10/11 change is required for v1.
...
- [ ] It cites the **actual** ENV-10 symbols (`NodeId`/`BaseNode.id`, the per-node `children[]`
      arrays, `RichTextJSON`) and ENV-11's index-based patch ops by name — not generic prose.
...
- Changing the ENV-10 types or ENV-11 patch format (this audit may *recommend* future changes
...
grep -Eq "NodeId|children|RichTextJSON" packages/doc-model/docs/crdt-readiness.md && echo "cites ENV-10 symbols"
