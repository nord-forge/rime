...
## Prerequisites
- ENV-40 done (`DndController` with per-`nodeId` cleanup map + `destroy()`;
  `syncCanvasTargets` idempotency). ENV-41's `DropDetector` (rAF) + indicator if
  merged — guard them too if present.

## Implementation notes
1. **Make cleanups countable** — `DndController` (and `DropDetector`,
   `<eb-drop-indicator>`, drag preview from ENV-43 if present) must funnel every
...
## Out of scope
- Rich-text one-instance memory (ENV-51) — separate §10 lever.
- Frame-rate / latency perf (ENV-42). Full heap profiling — counts are the proxy.
