...
`registerToken`/token-source config, ENV-72) and inserts the chosen one into the live
editor via ENV-70's `insertToken`. No library toolbar — ours.
...
## Prerequisites
- ENV-70 done (`insertToken(token, label?)`, the token node + chip render).
- The available token list. ENV-72 supplies it; until then, accept a `tokens` property
  and document the wiring to the token registry. Shape: `{ key: string; label: string;
  source?: string }[]`.
- ENV-52 (the inline rich-text toolbar) is the natural place to surface the trigger —
...
1. **`token-picker.ts`** — `EbTokenPicker extends LitElement` (`eb-token-picker`):
   ```ts
   @property({ attribute: false }) tokens: TokenItem[] = []; // from ENV-72 registry
   // emits nothing to the doc directly — calls insertToken() (ENV-70) on select
   ```
...
2. **Trigger + positioning.** Two entry points:
   - a button in the inline rich-text toolbar (ENV-52),
   - optionally a `{{` autocomplete trigger: when the user types `{{` in the editor,
     ...
     complicates ENV-50's IME handling — note the Safari/IME caution from R-1).
   Anchor the popover to the caret/selection rect; clamp to viewport. Reuse ENV-52's
...
4. **One-instance discipline.** The picker operates on the single live editor instance
   (ENV-51); if no text block is focused, the picker trigger is disabled.
...
## Out of scope
- The token node/data + `insertToken` (ENV-70). The token registry/config (ENV-72) —
...
## Definition of done
See `_conventions.md`. Custom themed token picker inserts via ENV-70; size gate
green; status → `review`.
