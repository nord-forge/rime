...
depends_on: []
blocks: [ENV-02, ENV-03, ENV-04, ENV-05, ENV-10, ENV-14, ENV-44, ENV-45, ENV-46]
package: root
prd: [§8]
estimate: M
---
...
The toolchain is already decided (OD-2): rolldown-vite + oxlint + oxfmt + tsc.
...
4. **Install the agreed runtime deps** at the right package:
   - `core`: `lit`, `@atlaskit/pragmatic-drag-and-drop`, `lexical` +
     ...
     `@lexical/utils` (engine decided in OD-4). **Do NOT install Tiptap.**
...
## Out of scope
- CI wiring (ENV-02). Playwright harness (ENV-03). Any feature code.
- The bundle-size GATE enforcement (ENV-02) — but DO copy `measure.ts` into a
  shared `scripts/` location so ENV-02 can wire it.
