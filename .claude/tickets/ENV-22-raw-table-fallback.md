...
# ENV-22 — Raw-table fallback path
...
that seam to the ENV-21 renderer and documents it so block authors know how to opt out of MJML for
...
## Prerequisites
- ENV-21 done (`MjmlRenderer`, the `to-mjml.ts` `BlockRenderer` registry, `RenderContext`).
- ENV-20's `BlockRenderer`/`RenderContext` types (the seam this builds on).
...
1. **`raw-fallback.ts`** in `packages/renderer-mjml/src/` — a small helper + a way for a block to
   ...
   ```
2. **Wire it into the ENV-21 registry.** `docToMjml`'s dispatch already looks up a `BlockRenderer`
...
- [ ] `rawTableFallback(html)` and `createRawBlockRenderer(type, fn)` exported and usable as a
      `BlockRenderer` in the ENV-21 registry.
...
- Authoring real raw-table HTML for any specific built-in block (the core blocks ship as MJML in
  ENV-21; this is the mechanism, not a catalog of fallbacks).
- The `registerBlock` SDK that custom blocks use end-to-end (ENV-60) — this only provides the
  export-side seam it will call.
- Client-correctness verification of fallback output (ENV-23 / manual matrix).
