...
# ENV-46 — Cross-browser DnD E2E (WebKit + touch)
...
regression net that lets ENV-40/41 changes ship safely.
...
## Prerequisites
- ENV-41 done (drop detection + insertion indicators; the user-visible behaviour
  to assert against).
- ENV-04 Playwright harness (base fixtures, `canvasFrame()` helper to reach the
  iframe document, http server so `srcdoc` is same-origin).
- A realistic newsletter doc fixture (reuse ENV-42's if present, else add one
...
4. **Wire into CI** — this suite runs under the existing `bun run e2e` (ENV-03 CI
...
## Out of scope
- Perf measurement (ENV-42), memory-leak verification (ENV-47).
- Keyboard/ARIA E2E (covered alongside ENV-44/45) — this ticket is pointer/touch.
