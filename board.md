...
| ~~ENV-02~~ | ✅ **DONE** — Toolchain spike. **rolldown-vite** builds + runs Lit/CSS/iframe (Chromium + WebKit smoke pass); oxlint/oxfmt Lit-safe; size gate prototyped. Stock Vite = fallback. OD-2 resolved. See `.claude/spikes/od2-toolchain/FINDINGS.md`. OD-4 budget number still pending in ENV-57. | P0 | ENV-01 | §7, OD-2/4 |
...
| ~~ENV-05~~ | ✅ **DONE** — Rich-text engine spike (OD-1). Decided **Tiptap/ProseMirror**. See `.claude/spikes/od1-richtext/FINDINGS.md`. | P0 | ENV-01 | §6.7, OD-1 |
...
| ID | Title | Pri | Depends on | PRD |
|----|-------|-----|------------|-----|
| ENV-50 | **Integrate Lexical headless in the canvas** (per OD-1/ENV-57) + **hard-confirm bundle budget**: measure real `@enveloppe/core` gzip; revert to Tiptap only if budget is raised. Start from `.claude/spikes/od1-richtext/src/lexical-adapter.ts` (registerRichText + curated nodes). | P0 | ENV-57, ENV-32 | §6.7, OD-1/4 |
...
| ~~ENV-56~~ | ✅ **DONE (premise disproven)** — curating Tiptap cannot hit < 90 kB; ProseMirror floor is ~108 kB, realistic v1 set is ~128 kB gzip (> StarterKit). Gap to Lexical (~43 kB) is structural ~3×. See `.claude/spikes/od1-richtext/ENV-56-FINDINGS.md`. | P1 | ENV-05 | §6.7, OD-4 |
