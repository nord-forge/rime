...
# ENV-45 — ARIA live-region announcements
...
ENV-44 left an `announce(msg)` hook on the keyboard-move path; this ticket builds
the actual polite live region and the message builder, and wires it to all three
structural events — move, insert, delete — from both the keyboard path (ENV-44)
and the pointer DnD path (ENV-40). Without spoken feedback the keyboard reorder
...
## Prerequisites
- ENV-44 done (`KeyboardMoveController` with the `announce` hook + selection).
- ENV-40 done (pointer drops via `DndController`; hook its drop completion to
  announce too).
- ENV-10 node types (to label a block by a human name + read its position).
...
3. **Wire the events:**
   - ENV-44 `KeyboardMoveController.announce` → `LiveAnnouncer.announce(moveMessage(...))`.
   - ENV-40 `DndController` drop completion → announce `move`/`insert` accordingly
...
## Out of scope
- The keyboard move semantics themselves (ENV-44) and pointer DnD (ENV-40).
