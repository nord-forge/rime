...
# ENV-23 — Output-correctness fixtures + client matrix
...
the §11 sign-off (ENV-102). This is the safety net for every future renderer change.
...
## Prerequisites
- ENV-21 done (`MjmlRenderer.render`, deterministic output).
- ENV-10 factory helpers to build the fixture docs cleanly.

## Implementation notes
Create under `packages/renderer-mjml/`:

1. **Fixture docs** — `test/fixtures/*.ts` (or `.json`). Build one doc per scenario covering the
   correctness-risky cases, using ENV-10 factories:
...
3. **Manual verification matrix** — `docs/email-client-matrix.md`. A table and a procedure:
   ...
   - **How to run it:** a documented procedure to produce the HTML files for sending — e.g. a small
     ...
     matrix doc is the checklist a reviewer fills in for the ENV-102 sign-off.
...
5. No new runtime dependency (test/dev tooling only). `mjml` is already an ENV-21 dep.

## Acceptance criteria
- [ ] At least five fixture docs exist covering text+marks+link, button, two-column, image/divider/
      spacer, and a full newsletter — all built via ENV-10 factories and `validateDoc`-valid.
...
## Out of scope
- The actual manual client sign-off run (ENV-102 executes the matrix on the release candidate).
- Visual-regression/screenshot diffing of real clients (manual matrix covers clients; snapshots
  cover HTML).
- Raw-table fallback fixtures (ENV-22) beyond what the core blocks already exercise.
...
human sign-off itself is ENV-102.)
