...
depends_on: [ENV-31, ENV-10]
blocks: [ENV-40, ENV-50, ENV-60]
package: core
prd: [§6.2]
estimate: L
---

# ENV-32 — Doc → canvas renderer (WYSIWYG preview DOM)

## Context
This is the WYSIWYG: it turns the `EnveloppeDoc` JSON (the single source of truth,
ENV-10) into the **clean preview DOM** inside the canvas iframe (§6.2). Critically,
...
email HTML is produced separately at export time (ENV-21, off the hot path). Updates
...
## Prerequisites
- ENV-31 done (`CanvasController` exposes `#eb-root` mount + `whenReady()`).
- ENV-10 done (`EnveloppeDoc`, node types, `validateDoc` from `@enveloppe/doc-model`).
- The doc is immutable and edits arrive as new docs / patches (ENV-11); the renderer
...
1. **`canvas/render-node.ts`** — pure functions mapping each node type to preview
   ...
   ```ts
   // Every rendered element carries its node id so hit-testing/selection (ENV-33/40)
...
3. **Incremental update strategy** — keep a `Map<NodeId, HTMLElement>`. On `update`:
   - Walk old vs new tree by id. **Unchanged** node (referentially equal subtree,
     courtesy of ENV-11 structural sharing) → skip entirely. **Changed props** →
...
4. **Text blocks** — render `RichTextJSON` (the portable shape from ENV-10's
   ...
   by ENV-50 — this ticket just paints the static representation; do not import
   Lexical here.
5. **Wire into the shell** — after `CanvasController.whenReady()`, construct
   ...
   changes, call `update(nextDoc)`. Expose `nodeIdAt`/`elementForNode` for ENV-33/40.
...
## Out of scope
- Email export HTML / MJML (ENV-21) — different render target entirely.
- Live rich-text editing instance (ENV-50) — only static text rendering here.
- Selection chrome, drop indicators, DnD (ENV-40/41), coordinate math (ENV-33).
- Block registration interface (ENV-60) — built-in types only for now.
