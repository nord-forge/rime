...
# ENV-44 — Keyboard reordering path (a11y)
...
through ENV-11 ops — the keyboard path and the pointer path converge on the same
mutation layer, so behaviour and undo stay identical. The spoken feedback
(ARIA live announcements) is ENV-45, built directly on this ticket's events.
...
applying an ENV-11 op, with focus following the moved block.

## Prerequisites
- ENV-40 done (canvas blocks rendered with `data-node-id`; ENV-11 `moveNode`
  available; `dispatch` applies op results to editor state + re-render).
- ENV-32 `CanvasRenderer.elementForNode(id)` to move focus to the moved element.
...
1. **`keyboard-move.ts`** — `KeyboardMoveController`, owning selection + keyboard
   ...
   export interface KeyboardMoveDeps {
     getDoc(): EnveloppeDoc;
     dispatch(op: OpResult): void;             // ENV-11 result → editor
     getSelected(): NodeId | null;
     setSelected(id: NodeId | null): void;
     focusNode(id: NodeId): void;              // elementForNode(id)?.focus()
     announce(msg: string): void;              // ENV-45 hook (no-op stub until then)
   }
   ...
   export class KeyboardMoveController {
     ...
     /** Apply a move via ENV-11 moveNode; updates selection + focus + announce. */
...
2. **Key bindings** — on the focused/selected block (modifier chosen to not clash
   ...
   - Do NOT intercept arrows when a Lexical editor is focused inside a TextBlock
     (ENV-50/51) — guard on whether the active element is the live editor.
3. **"Move to" menu** — `<eb-move-to-menu>` Lit component: a small popover listing
   ...
   `Esc` to close).
4. **Every move = ENV-11** — `move()` calls `moveNode(doc, id, target.parentId,
...
- [ ] Every move goes through ENV-11 `moveNode`; no direct DOM mutation.
- [ ] `announce(...)` is called with a message on each move (content filled by
      ENV-45; here a hook + reasonable default string).

## Out of scope
- The actual ARIA live-region wiring + message wording polish (ENV-45).
- Pointer DnD (ENV-40), indicators (ENV-41), perf (ENV-42).
...
## Definition of done
See `_conventions.md`. Keyboard reordering + move-to menu, all via ENV-11,
