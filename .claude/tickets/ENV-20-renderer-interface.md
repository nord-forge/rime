...
depends_on: [ENV-10]
blocks: [ENV-21, ENV-22]
package: renderer-mjml
prd: [§6.3]
estimate: S
---

# ENV-20 — Swappable Renderer interface

## Context
Export is decoupled from MJML behind a swappable **`Renderer` interface** (PRD §6.3): the v1
default is `MjmlRenderer` (ENV-21), but a future hand-rolled renderer must be a **non-breaking
swap, not a rewrite**. This ticket defines that interface (and supporting types) so ENV-21 and
the per-block fallback (ENV-22) implement a contract rather than an ad-hoc function. The
...
## Prerequisites
- ENV-10 done (`EnveloppeDoc` type to render against; imported via `workspace:*`).
...
1. **`renderer.ts`** — the contract. Keep it minimal and engine-agnostic:
   ...
   ```
2. **Per-block seam (declare here, implement later).** The block → output mapping that ENV-21
   fills in and ENV-22 escape-hatches should be expressed as a typed seam so both tickets share
   one shape. Declare it here but leave it un-implemented:
   ```ts
   import type { EnveloppeDoc } from "@enveloppe/doc-model";
   type AnyNode = EnveloppeDoc | EnveloppeDoc["children"][number]; // narrow per node in ENV-21
   ...
   ```
   ENV-21 wires a registry of `BlockRenderer`s; ENV-22 registers a raw-table `BlockRenderer` for
...
## Out of scope
- The actual `MjmlRenderer` and any block → MJML mapping (ENV-21).
- The raw-table fallback implementation (ENV-22) — only the `BlockRenderer` seam is declared here.
- Rich-text → HTML conversion (ENV-21).
