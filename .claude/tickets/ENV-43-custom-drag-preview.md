...
"this block". This is a small, self-contained UI ticket on top of ENV-40's wiring.
...
## Prerequisites
- ENV-40 done (`DndController`, `DragData`, palette + canvas draggables).
- `--eb-*` theme tokens (ENV-34) for styling.
...
3. **Cleanup** — the `render` return MUST remove the preview element; verify no
   detached preview nodes accumulate across repeated drags (ENV-47 territory, but
   keep it clean here).
...
## Out of scope
- Drop indicators (ENV-41), perf budget (ENV-42), keyboard/a11y (ENV-44/45).
- Palette icon/label registration shape (ENV-63) — consume whatever metadata the
