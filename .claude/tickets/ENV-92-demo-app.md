...
# ENV-92 — Demo app (proves end-user UX)
...
(`<enveloppe-editor>` + ENV-80 JSON in/out + ENV-81 `onImageUpload` stub + `--eb-*`
theming), uses a trivial **local store** (no backend, per §6.10), and showcases
theming. It registers the example custom block (ENV-64) to prove the SDK in context.
...
## Prerequisites
- ENV-61 done (core blocks render/drag/edit) and ENV-80 done (`loadDoc`/`getDoc`/
  `change`). ENV-81 (`onImageUpload`) and ENV-64 (example block) land here if ready;
...
2. **Local store (headless persistence demo).**
   - On `change`, `serialize(e.detail.doc)` (ENV-14) → `localStorage`.
...
3. **`onImageUpload` stub.** A callback that returns an object-URL (`URL.createObjectURL`)
   or a tiny data-URI — proving the host-uploader contract (ENV-81) with **no backend**.
...
4. **Theme showcase.** A switcher that applies different `--eb-*` token sets (e.g.
   ...
   canvas (email styles) is unaffected by chrome theme (two-surface model, ENV-34).
5. **Export preview.** A "View HTML" panel that runs `new MjmlRenderer().render(getDoc())`
   (ENV-21) and shows the email HTML — closing the loop from edit → bulletproof output.
6. **Custom block.** Register the ENV-64 example ("Coupon") so it appears in the palette
...
- [ ] The ENV-64 example custom block appears in the palette and works.
...
- Polished marketing site (that's docs, ENV-103). React/Vue-specific demos (the
