...
export class RichTextToolbar extends LitElement {
  static styles: CSSResultGroup = css`
    ...
    .sep {
      inline-size: 1px;
      background: var(--rime-color-border, #e4e4e7);
      margin: 2px 2px;
    }
  `;
  ...
  requestLink(): void {
    this.dispatchEvent(
      new CustomEvent<{ href: string | null }>("rime-request-link", {
  ...
  requestToken(): void {
    this.dispatchEvent(new CustomEvent("rime-request-token", { bubbles: true, composed: true }));
  }
...
if (!customElements.get("rime-rich-text-toolbar")) {
  customElements.define("rime-rich-text-toolbar", RichTextToolbar);
}
