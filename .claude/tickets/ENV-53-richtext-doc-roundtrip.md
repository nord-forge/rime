...
# ENV-53 — Lexical ↔ doc-model RichTextJSON round-trip

## Context
The doc model is the single source of truth and stores text as **portable
`RichTextJSON`** (ENV-10's `rich-text.ts`) so it never depends on Lexical (§6.1).
The live Lexical editor (ENV-50) has its own internal editor state; this ticket
...
doc via ENV-11 `setRichText`. If this round-trip drops a mark or a link, edits
...
the content is committed back into the doc via ENV-11 `setRichText`.

## Prerequisites
- ENV-50 done (`mountLexical`, `LexicalMount.toJSON()`, `{ discrete: true }` seed).
- ENV-51 (the `onCommit(nodeId, json)` blur hook to call into).
- ENV-14 done (serialize/deserialize — the doc round-trips losslessly to JSON; this
  ticket guarantees the *text* part of that is lossless through the editor too).
- ENV-11 `setRichText(doc, textBlockId, RichTextJSON)` → `{ doc, patch, inverse }`.
- ENV-10 `RichTextJSON` (paragraphs → runs with `marks` + `link`; plus heading /
  list block kinds — extend the portable shape if ENV-10 didn't include them, in
...
1. **`serialize.ts`** — the two pure converters, the single source of mapping
   truth (ENV-50's seed + `toJSON` should call into these rather than duplicating):
   ```ts
   ...
   - **Lists:** `ListNode`/`ListItemNode` ↔ a list block shape
     (`{ type: "list", ordered, items: paragraph[] }` or the shape ENV-10 defines).
...
2. **Blur commit** — implement the `onCommit(nodeId, json)` that ENV-51 calls on
   blur:
   ```ts
   const op = setRichText(getDoc(), nodeId, json);
   dispatch(op); // editor merges patch into doc + undo history (ENV-11/12)
   ```
...
4. **Reuse in ENV-50** — refactor ENV-50's seed + `toJSON` to call
...
- [ ] On TextBlock blur, content is committed into the doc via `setRichText`
      (ENV-11), producing a patch + undo entry, only when changed.
- [ ] ENV-50's seed + `toJSON` are refactored to use these converters (one mapping).
- [ ] Works in Chromium + WebKit (the discrete-update read path is engine-correct
      in both, per the spike).

## Out of scope
- The lifecycle/focus-blur eventing (ENV-51) — this ticket implements the
  `onCommit` body it calls.
- Paste sanitization (ENV-54). Merge tags (ENV-70).
- Doc-level JSON file serialize (ENV-14) — this is only the text-content bridge.
...
ENV-11, cross-browser; status → `review`.
