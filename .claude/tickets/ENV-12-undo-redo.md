...
# ENV-12 — Undo/redo history stack

## Context
The editor needs undo/redo, and the hard memory constraint (potato-PC target) forbids
storing full-document snapshots. ENV-11 already produces a `{ patch, inverse }` pair for
...
## Prerequisites
- ENV-11 done (`applyPatch`, `invertPatch`, and the operations returning `{ doc, patch, inverse }`).
- Reuse ENV-11's `Patch` type and `applyPatch`; do NOT re-derive patches here.

## Implementation notes
Create in `packages/doc-model/src/`:

1. **`history.ts`** — the history container. It is the **owner of the current doc**: callers
   push the result of an ENV-11 operation, and the history tracks the resulting tree so
...
3. **Coalescing.** Only coalesce when: `coalesceKey` is defined and equal to the previous
   ...
   `"text:" + nodeId`). The editor (ENV-50/53) supplies the key; this ticket only honors it.
...
4. **Convenience wrapper (optional but recommended).** Add `record(result, coalesceKey?)`
   taking an ENV-11 op result `{ doc, patch, inverse }` so callers don't destructure:
   ```ts
...
5. **Purity / determinism.** No `Date.now()` / `Math.random()` at module top level; the clock
   is injectable via `options.now` (mirrors ENV-10's id-factory rule). `History` holds mutable
...
## Out of scope
- Producing patches/inverses (ENV-11 owns that) — this ticket only stacks them.
- Wiring undo to keyboard shortcuts or the editor UI (Milestone 3+).
- Choosing real coalesce keys for specific editor interactions (ENV-50/53 supply them).
