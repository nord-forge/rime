...
depends_on: [ENV-50]
blocks: [ENV-71, ENV-72]
package: core
prd: [§6.9]
estimate: M
---

# ENV-70 — `{{variable}}` merge tags in rich text
...
survives the portable `RichTextJSON` round-trip (ENV-53), and exports to literal
`{{var}}` in the email HTML. This is the data + render foundation; the picker UI
(ENV-71) and `registerToken` config (ENV-72) build on it.
...
- ENV-53/ENV-10 (`RichTextJSON`: paragraphs → runs with `marks`/`link`) — the portable
  shape this extends.
- ENV-21 (`richTextToInlineHtml` — must learn to emit token output).
...
3. **Editor ↔ RichTextJSON round-trip.** Extend the ENV-53 adapter so:
   - Lexical token node → `TokenInline` on serialize,
   - `TokenInline` → Lexical token node on load.
   Assert a doc with mixed text + tokens round-trips byte-equivalent (ENV-14 law).
4. **Insertion API.** Export `insertToken(token: string, label?: string)` that inserts a
   token node at the current selection in the live editor (the picker UI in ENV-71
   calls this). One live editor instance only (ENV-51) — operate on the active one.
...
6. **Canvas preview (ENV-32 static paint).** The read-only canvas paint of a text block
...
- [ ] A Lexical token node renders an atomic, themed chip and serializes to/from
      `TokenInline` via the ENV-53 adapter.
...
## Out of scope
- The token picker UI (ENV-71) — this provides `insertToken`, not the menu.
- `registerToken` / token-source config (ENV-72) — token *sets* come from there.
