...
# ENV-52 — Custom rich-text UI (toolbar / bubble / link popover)
...
ENV-50 mounted **inside the iframe**. The UI reads the current selection's format
...
## Prerequisites
- ENV-50 done (Lexical mounted in-iframe; access to the active `LexicalEditor`).
- ENV-51 (the active editor / `activeNodeId`) so the toolbar targets the right
  instance and hides when no editor is live.
- ENV-33 coordinate controller (`canvasToHost`) to position the bubble toolbar
  over the in-iframe selection.
- `--eb-*` theme tokens (ENV-34).
...
1. **`rich-text-commands.ts`** — a thin command facade so UI never touches Lexical
   ...
   `LinkNode` (`@lexical/link`) — add these nodes to the curated set in ENV-50's
...
## Out of scope
- Lifecycle (ENV-51), doc round-trip on blur (ENV-53), paste (ENV-54).
- Merge-tag/token picker (ENV-70/71). Slash/insert menu (future).
