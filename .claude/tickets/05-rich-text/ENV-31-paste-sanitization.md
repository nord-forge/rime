...
# ENV-31 — Paste sanitization (Word/Outlook/Docs → schema-valid)
...
## Prerequisites
- ENV-27 done (Lexical mounted with `registerRichText(editor)` + curated nodes
  `[HeadingNode, QuoteNode]` (+ Paragraph/Text), plus List/Link nodes from ENV-29
...
- ENV-30's `$readRichTextJSON` to assert the *resulting doc content* is clean.
...
3. **Harden the edges** the spike didn't exhaustively cover:
   - **Links in paste:** an `<a href>` should survive as a link ONLY if `LinkNode`
     is registered (ENV-29); a `javascript:`/`data:` href must be stripped or the
     link dropped (validate href on import — reuse ENV-29's href normalizer).
...
## Out of scope
- New rich-text features (ENV-29). Doc round-trip law (ENV-30).
- IME/mobile/Safari manual hardening (ENV-32).
