...
# ENV-38 — Social block (icons + links)
...
`registerBlock` path as the core blocks (ENV-34) — no special-case path. MJML has a
...
- ENV-05 may need a small node-type addition for `social` (a `links: { network; href }[]`
...
2. **`schema`.** A repeatable list field for links (network `select` + `url`), plus
   align/spacing/icon-size. If ENV-35 lacks a "list" field type, add a minimal
...
6. **Register** in `registerCoreBlocks()` (ENV-34) but allow it to be excluded via
...
- The list-field control if it must be built in ENV-35 — coordinate, don't duplicate.
