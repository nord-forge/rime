...
# ENV-51 — One live Lexical instance lifecycle
...
live contenteditable editors would be a memory + listener disaster. ENV-50 gave
...
## Prerequisites
- ENV-50 done (`mountLexical(blockEl, initial)` → `LexicalMount` with `destroy()`).
- ENV-32 (TextBlock elements rendered with `data-node-id` in the iframe;
  `elementForNode`).
...
1. **`richtext-lifecycle.ts`** — `RichTextLifecycle`, the single owner of the (at
   ...
   export class RichTextLifecycle {
     ...
     constructor(private deps: {
       ...
       onCommit(nodeId: NodeId, json: RichTextJSON): void; // ENV-53 blur write-back
     }) {}
     /** Focus a text block: tear down any active editor, mount on this one. */
     focus(nodeId: NodeId): void;
     /** Blur: serialize + commit (ENV-53) then destroy the active editor. */
     blur(): void;
...
4. **Blur write-back hook** — on `blur()`, before `destroy()`, call
   `mount.toJSON()` and `deps.onCommit(nodeId, json)` so ENV-53 serializes back
   into the doc via ENV-11 `setRichText`. (ENV-53 implements `onCommit`; here just
   call the hook.)
...
6. **Leak discipline** — `destroy()` and every `blur()` fully release the previous
   mount (ENV-50's `destroy` already does `cleanup()` + `setRootElement(null)`);
...
## Out of scope
- The actual serialize-into-doc implementation (ENV-53 `setRichText`) — only the
  `onCommit` hook call here.
- Toolbar/bubble UI (ENV-52), paste (ENV-54), IME/Safari nuances (ENV-55).
