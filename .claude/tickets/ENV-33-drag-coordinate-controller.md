...
# ENV-33 — Coordinate-translation drag controller
...
R-2). This ticket builds that controller as the foundation ENV-40 (Pragmatic DnD)
...
## Prerequisites
- ENV-31 done (`CanvasController` gives the iframe + its `contentDocument`).
- ENV-32's `nodeIdAt`/`elementForNode` exist for resolving points to nodes (this
  ticket may consume them; if ordering lands ENV-33 first, resolve via
...
6. **No DnD here** — this controller is pure coordinate/hit-test math. Pragmatic
   drag-and-drop integration, drop zones, and indicators are ENV-40/41. Keep this
...
## Out of scope
- Pragmatic drag-and-drop wiring, drag previews (ENV-40/43).
- Drop-zone detection + insertion indicators (ENV-41).
