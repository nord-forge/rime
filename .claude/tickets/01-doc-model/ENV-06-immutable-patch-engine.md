...
# ENV-06 — Immutable update + patch-diff engine
...
patches — not full-document snapshots — are what undo/redo (ENV-07) stores, which
...
## Prerequisites
- ENV-05 done (types + validate + factory).
...
## Out of scope
- The undo/redo STACK + history cap (ENV-07) — this ticket only produces the
  patch+inverse pairs it will store.
- Serialization (ENV-09).
