...
# ENV-51 — Docs site / API reference
...
2. **Pages (the four surfaces):**
   ...
   - **`registerBlock` (custom blocks SDK):** document `BlockDefinition`
     ...
     (ENV-33), the canvas vs export render contexts, and the raw-table fallback. Embed
...
4. **Cross-links.** Link to the demo (ENV-46) and the embed-test note (ENV-48). Surface
...
- [ ] The site builds/serves via a documented command and is referenced by ENV-52.
...
theming with the ENV-37 walkthrough; status → `review`.
