...
depends_on: [ENV-20]
blocks: [ENV-22, ENV-23, ENV-60]
package: renderer-mjml
prd: [§6.3]
estimate: L
---

# ENV-21 — MjmlRenderer (doc → MJML → HTML)
...
budget** (it lives in its own package). `MjmlRenderer` implements the ENV-20 `Renderer` contract.
...
## Prerequisites
- ENV-20 done (`Renderer`, `RenderOptions`, `RenderError`, `BlockRenderer`, `RenderContext`).
- ENV-10 done (the node types being mapped: `document/section/column/text/image/button/divider/spacer`,
...
2. **`to-mjml.ts`** — the doc → MJML-string mapper, built as a registry of `BlockRenderer`s
   (the ENV-20 seam) so ENV-22 can register a raw-table renderer per block. One handler per
   ...
   - Containers (`document`/`section`/`column`) delegate to children via `ctx.renderChild` so the
     registry stays the single dispatch point (this is also the seam ENV-22 hooks).
3. **`rich-text-to-html.ts`** — `richTextToInlineHtml(content: RichTextJSON): string`. Walk the
   portable rich-text shape (ENV-10: doc → paragraphs → text runs with
...
4. **Unknown node types.** If a node's `type` has no registered handler, throw `RenderError`
   naming the type and its path (the raw-table fallback in ENV-22 is how a block opts out of MJML;
...
5. **Determinism.** Same doc → same HTML string (modulo MJML's own output). No `Date.now()`/random
   in the mapper. This is what makes ENV-23's golden snapshots stable.
...
## Out of scope
- The per-block **raw-table fallback** path (ENV-22) — only the `BlockRenderer` registry seam it
  plugs into is built here.
- Golden-snapshot fixtures + the manual email-client verification matrix (ENV-23).
- The block-registration SDK (`registerBlock`, ENV-60); built-ins are hardcoded handlers here.
