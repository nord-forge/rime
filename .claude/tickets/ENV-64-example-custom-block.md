...
# ENV-64 — Example custom block (the SDK proof)
...
doubles as the canonical copy-paste template the docs (ENV-103) reference. The example
...
## Prerequisites
- ENV-60 done (`registerBlock`, `BlockDefinition`, schema/palette/render contexts —
...
2. **The block** — a "Coupon" block (a marketing block MJML can't fully express, so it
   ...
   registerBlock({
     ...
     // raw-table fallback: returns { raw } so ENV-22 passes it through verbatim
   ...
   - `renderExport`: emit a **raw email-safe `<table>`** (the ENV-22 fallback) since a
...
3. **Heavy inline documentation.** This file is a teaching artifact: comment every part
   ...
   The docs site (ENV-103) embeds it; keep it self-explanatory.
4. **Consume it.** Register the example in the demo app (ENV-92) so it actually appears
...
5. **No core changes.** If implementing this requires reaching into core internals, that
   is a **bug in ENV-60's public surface** — fix the export there, do not add a
...
- [ ] `renderExport` uses the `{ raw }` raw-table path (exercising ENV-22), with all
      text escaped.
- [ ] The example is registered/used by the demo app and works end to end there.
- [ ] Heavy inline comments make it usable as the docs (ENV-103) reference template.
- [ ] If anything needed a non-public import, ENV-60's public surface was fixed instead.

## Out of scope
- Shipping this block inside `@enveloppe/core` (it's a sample, not a built-in).
- The docs site itself (ENV-103) — this just supplies the canonical example.
- Social block (ENV-65).
