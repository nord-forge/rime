...
# ENV-81 — `onImageUpload` host callback (image block)
...
package. This ticket wires the image block (ENV-61) to that callback (already typed on
`EnveloppeConfig`, ENV-30). The demo (ENV-92) provides a stub uploader to be runnable.

## Goal
The image block lets a user pick a file, calls `config.onImageUpload(file)`, and stores
the returned URL into the block's `src` via an ENV-11 op — with no storage logic in the
library and graceful handling when no callback is provided.

## Prerequisites
- ENV-61 done (the `image` block: `ImageBlock { src; alt; href?; style }`).
- ENV-30's `EnveloppeConfig.onImageUpload?: (file: File) => Promise<string>`.
- ENV-11 ops (to write `src` into the doc immutably) and ENV-62 (the image block's
  properties form where the upload control lives).

## Implementation notes
1. **Upload affordance.** In the image block's properties form (ENV-62) and/or an
...
2. **Call the host.** On file selection, call `config.onImageUpload(file)`:
   ...
   if (url) setNodeProp(doc, imageId, "src", url); // ENV-11 op → change event
   ```
...
- [ ] Selecting a file in the image block calls `config.onImageUpload(file)` and stores
      the returned URL into `src` via an ENV-11 op (emitting `change`).
...
## Out of scope
- Any real upload/storage implementation (that's the host's; the demo ENV-92 supplies a
  stub).
