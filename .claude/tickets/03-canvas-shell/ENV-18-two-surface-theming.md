...
by `--rime-*` CSS custom properties that intentionally pierce shadow boundaries, and
...
chrome `--rime-*` token bleeds into the canvas**. The bleed-proof is the whole point —
...
## Goal
Document and enforce the two-surface model: `--rime-*` themes chrome only; the canvas
is styled solely by its injected base stylesheet; a Playwright test proves neither
host CSS nor `--rime-*` reaches inside the canvas — in Chromium + WebKit.

## Prerequisites
- ENV-14 done (`--rime-*` applied to the shell host; chrome regions exist).
...
1. **Token catalogue** — create `packages/core/src/theme/tokens.ts` documenting the
   ...
   `color: var(--rime-color-accent, #5b5bd6)` — never hard-codes the themed value.
2. **Chrome channel** — confirm `RimeEditor` applies `config.theme` overrides as
   `--rime-*` on the host (ENV-14). No host stylesheet is ever read or copied.
3. **Canvas channel** — the iframe's only styles come from `#rime-base`
   (`CanvasController.setBaseStyles`, ENV-15) plus what ENV-16 writes into `#rime-root`.
   The canvas does **not** receive `--rime-*` tokens: explicitly do not forward chrome
...
4. **The no-bleed test** — the deliverable that matters. Create
   ...
   - On the **host** page, inject aggressive global CSS: `* { color: red !important;
     font-family: "Comic Sans MS" !important; }` and set a wild `--rime-color-accent`.
   - Assert chrome **does** pick up `--rime-color-accent` (token piercing works).
   - Assert content inside the canvas iframe (`#rime-root`) is **NOT** red and **NOT**
     Comic Sans, and that `getComputedStyle(canvasEl).getPropertyValue("--rime-color-accent")`
...
## Acceptance criteria
- [ ] `theme/tokens.ts` enumerates `--rime-*` chrome tokens + defaults and is the
      catalogue chrome components consume (with `var(--rime-*, default)` fallbacks).
- [ ] Chrome visibly responds to a `--rime-*` override (token piercing confirmed).
- [ ] Host CSS (`* { color:red !important }`, font override) does **not** affect
      content inside the canvas iframe.
- [ ] `--rime-*` tokens set on the host do **not** resolve inside the canvas document.
...
cd packages/core
bun run build
bun run e2e   # theming.spec: chrome responds to --rime-*; host CSS + tokens do NOT reach the canvas — chromium + webkit
bun run lint
```
