...
# ENV-55 — IME / mobile / Safari rich-text hardening
...
## Prerequisites
- ENV-50 done (Lexical mounted in-iframe), ENV-51 (focus/blur lifecycle), ENV-52
  (custom toolbar — its positioning is a Safari/mobile risk), ENV-53 (round-trip
...
1. **`packages/core/docs/RICHTEXT-QA-CHECKLIST.md`** — the executable manual
   ...
   - **CJK IME composition:** type with Pinyin/Japanese IME — the composition
     ...
     destroy via the focus lifecycle (ENV-51 must not blur mid-composition). The
     round-trip (ENV-53) must not drop composed characters.
   ...
   - **iOS selection + toolbar:** selecting text shows the native callout AND our
     ...
     selection (ENV-52's `canvasToHost`) despite iOS visual-viewport offset; the
   ...
   - **Mobile paste:** long-press paste of formatted content sanitizes (ENV-54)
     the same as desktop.
...
2. **Fix surfaced issues** — for each failure, implement the fix in the relevant
   module (likely ENV-51 lifecycle guarding `isComposing`, ENV-52 toolbar
   positioning using `window.visualViewport`, or focus handling in ENV-50). Add a
...
3. **Composition guard (most likely fix, do proactively)** — ensure the lifecycle
   ...
   document) into ENV-51's lifecycle so a blur mid-composition is deferred.
4. **`packages/core/docs/RICHTEXT-QA-FINDINGS.md`** — record device/OS/IME
   ...
   commits/tickets). This is the sign-off artifact R-1 points at and ENV-101 can
   cite.
...
## Out of scope
- Engine choice re-litigation (decided: Lexical, OD-1). Paste mechanism (ENV-54) —
  only its mobile-paste behaviour is checklisted here.
- Non-text DnD touch behaviour (ENV-46).
