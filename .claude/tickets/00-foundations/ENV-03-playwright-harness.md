...
# ENV-03 — Shared Playwright harness + editor fixtures
...
ticket (ENV-15/32/33/34/40+/50+) will build its specs on, so those tickets write
...
- ENV-01 done (core builds to `dist/`, `lit` resolvable). The editor shell itself
  (ENV-14) may not exist yet; the harness must work against a minimal placeholder
...
4. **Iframe-canvas helpers** — provide `canvasFrame()` returning a `FrameLocator`
   ...
   coordinate controller (ENV-17) — do not implement coordinate math here.
...
7. **CI note** — ENV-02 intentionally omits browser binaries. Document in the
   ticket's PR how `bun run e2e` would be added to a separate CI job (Playwright
   install of chromium + webkit) if desired; do not wire it into ENV-02's fast job.
...
- [ ] Harness works against a placeholder element (unblocked by ENV-14) and requires
      no fixture change when the real shell lands.

## Out of scope
- Real coordinate-translation helpers (ENV-17) — only a documented stub here.
- DnD-driving helpers (ENV-19/46) and rich-text-driving helpers (ENV-27+).
- Wiring E2E into the ENV-02 CI job (note only).
