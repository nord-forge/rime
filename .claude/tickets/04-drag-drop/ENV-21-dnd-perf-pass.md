...
# ENV-21 — DnD performance pass + drop-detection budget (OD-3)
...
drop-detection budget, then measuring ENV-19/41's DnD against it and documenting
...
## Prerequisites
- ENV-20 done (rAF-gated `DropDetector`, geometry snapshot, insertion indicator).
- The ENV-03 Playwright harness (for instrumented browser runs).
...
3. **Reference-machine profile** — the low-end target. Approximate it in CI by
   ...
   hardware spec used for the manual sign-off (ENV-49 consumes this).
4. **Tighten if over budget** — if a gate fails, optimise WITHIN ENV-19/41's
...
5. **`packages/core/PERF-DND-FINDINGS.md`** — record: chosen OD-3 ms budget,
   ...
   p50/p95 detection latency, and any optimisations applied. ENV-49 (perf
...
## Out of scope
- Memory-leak verification (ENV-26) — a separate §10 gate.
- Cross-browser correctness E2E (ENV-25). Rich-text perf (ENV-28).
