...
## Prerequisites
- ENV-90 done (React wrapper — the most common embed path) and ENV-92 done (demo + the
...
1. **Set up a clean consumer.** Outside the monorepo (or a throwaway dir), create a
   minimal app (React via the wrapper, and/or vanilla per ENV-93). Install
...
6. **Cross-check both paths.** At minimum the React wrapper path; ideally also the
   vanilla path (ENV-93) since they share the README.
...
## Out of scope
- Perf/output-correctness gates (ENV-101/102) — this is the DX gate only.
- npm publishing (ENV-104) — uses a local tarball/link to simulate a consumer.
