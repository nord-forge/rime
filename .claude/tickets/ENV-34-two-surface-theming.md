...
# ENV-34 — Two-surface theming + no-CSS-bleed guarantee
...
## Prerequisites
- ENV-30 done (`--eb-*` applied to the shell host; chrome regions exist).
- ENV-31 ideally done (the iframe canvas + `setBaseStyles`); if ENV-31 isn't merged,
  the bleed test can target a minimal srcdoc iframe and be tightened when ENV-31 lands.
...
2. **Chrome channel** — confirm `EnveloppeEditor` applies `config.theme` overrides as
   `--eb-*` on the host (ENV-30). No host stylesheet is ever read or copied.
3. **Canvas channel** — the iframe's only styles come from `#eb-base`
   (`CanvasController.setBaseStyles`, ENV-31) plus what ENV-32 writes into `#eb-root`.
...
4. **The no-bleed test** — the deliverable that matters. Create
   `packages/core/e2e/theming.spec.ts` (using the ENV-04 fixtures):
...
## Out of scope
- The iframe canvas implementation itself (ENV-31) and doc rendering (ENV-32).
- A dedicated canvas/email-theme channel (future ticket) — v1 canvas styles come
  only from the injected base stylesheet.
- Properties-panel / palette visual design (ENV-62/63).
