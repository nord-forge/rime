...
# ENV-72 — registerToken / token-source config
...
`config.tokenSources` (JSON at init, already typed on `EnveloppeConfig` in ENV-30) and
**programmatic** via `registerToken`. Both feed one token registry that the picker
(ENV-71) reads. No conditionals/loops — just a named, labeled token set.
...
## Prerequisites
- ENV-70 done (the token node + `insertToken`; tokens are inserted by `key`).
- ENV-30's `EnveloppeConfig.tokenSources` (`TokenSource { id; label; tokens: { key;
...
1. **`registry.ts`** — mirror the block registry’s shape (consistency is the point):
   ...
   - `bySource()` is what ENV-71 groups by; flat `all()` for search.
2. **Config merge at init.** In the editor (ENV-30 lifecycle), when `config.tokenSources`
...
3. **Feed the picker.** Pass `tokenRegistry.all()` (or `bySource()`) into
   `<eb-token-picker>` (ENV-71). The picker stays presentational; the registry is the
   source of truth for "what tokens exist".
4. **Validation.** A token `key` must be a safe identifier-ish string (the substring
   that ends up inside `{{ }}` on export, ENV-70). Reject empty/whitespace keys.
5. **Export** `registerToken`, `registerTokenSource`, `tokenRegistry`, `TokenRegistry`,
   ...
   `TokenSource` type with ENV-30's identical interface (single definition, re-exported).
...
- [ ] The token picker (ENV-71) is driven by the registry contents (grouped by source).
...
## Out of scope
- The picker UI (ENV-71) and the token node/insert (ENV-70).
